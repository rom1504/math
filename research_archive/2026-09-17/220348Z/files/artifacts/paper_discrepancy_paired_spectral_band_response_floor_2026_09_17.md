# Sharp response floor for an isotropic Gaussian-sign spectral band
...
### The same sharp floor holds without the equal-pair restriction

In fact let nu be ANY mixture of centered Gaussian-sign laws whose
latent correlations R have spectra in [1-a,1+a], and assume only that
the resulting physical mixture is exactly isotropic. Its components
need not come in opposite or equal-weight pairs. Apply (2) to T=R-I:
the physical sign covariance C_R has

```
(1-s_a)I <= C_R <= (1+s_a)I.
```

For a fixed query x, the component normalized variances
z_R=x^T C_R x/n lie in [1-s_a,1+s_a]. Exact isotropy gives E_R z_R=1.
The chord below the concave function sqrt(z) on this interval yields

```
E_R sqrt(z_R)
 >= [sqrt(1-s_a)+sqrt(1+s_a)]/2=f(s_a).
```

The uniform scalar comparison can be averaged over the mixture,
proving (1) for this LARGER class as well. The matching pairs below
already attain the lower bound, so it stays sharp. Unequal weights
or nonpaired mixing within the same latent band cannot evade it.
...
It does not rule out non-Gaussian physical laws, Gaussian mixtures
outside this fixed band, structured dependent columns, rare selected
realizations, or a different parent construction.
...
### Adaptive children with bounded normalized cap

There is a complementary statement even when D is selected after
seeing the bridge, provided Q(D)<=L q^(3/2) for a fixed L. The elementary
deterministic inequality is

```
Q(parent)>=Q(A)+sum_j |h_j.x_0|-Q(D).
```

Each column law in the fixed band has subGaussian proxy (1+a)I.
Symmetrization and reverse triangle therefore give the centered absolute
field variance proxy 2(1+a)n. For independent columns, with probability
at least 1-exp[-q t^2/(4(1+a))],

```
Q(parent)>=Q(A)+[beta(a)-epsilon_n(a)-t]q sqrt(n)
                              -L q^(3/2).           (7a)
```

At q=epsilon n, the last term is of order epsilon^(3/2)n^(3/2),
so the same wrong linear slope follows by taking n first, then t down
to zero and epsilon down to zero. This extension was independently
supplied and audited by the localization researcher. It still does
not exclude exceptional deterministic bridges or unrestricted adaptive
children whose caps grow like q^2.
