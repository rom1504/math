...
covariance argument. **Audited valid by all three researchers, but its
bounded-operator conclusion is redundant with a stronger earlier affine
construction; see the discrepancy track's collision audit in Section 7.**
...
## 7. Independent collision audit: the stated conclusion is already stronger by an affine law

The discrepancy researcher independently reconstructed every proof step
above and the imported scalar replacement and Gaussian Holder results.
The theorem is correct. However, its CURRENT bounded-operator-norm
conclusion is not a new improvement: Section 3 of
`paper_localization_gaussian_sign_response_2026_09_17.md` already gives
a stronger explicit result under precisely the same hypothesis.

Indeed choose, without referring to a dual law mu,

```
R_plus/minus=I plus/minus A/(2L sqrt(n)).
```

These are correlation matrices with spectrum in[1/2,3/2]. Their equal
Gaussian-sign mixture is centered, exactly isotropic, and has the same
linear subGaussian proxy3/2. Because every off-diagonal A_ij is exactly
plus or minus one, its conditional sign covariance shift is EXACTLY

```
V=kappa^2 arcsin(1/(2L sqrt(n))) A.
```

For every x in the complete high-energy set C,

```
|x^T Vx/n|
 =2kappa^2 arcsin(1/(2L sqrt(n))) |H_A(x)|/n
 >=kappa^2 c/L.
```

The same uniform scalar comparison therefore gives, for this ONE
explicit law and ALL x in C,

```
E|h dot x|/sqrt(n)
 <=kappa f(kappa^2 c/L)+O(n^(-1/6)sqrt(log(n+1)))
 <=kappa-kappa^5 c^2/(8L^2)+O(n^(-1/6)sqrt(log(n+1))).
```

This discount is larger than (2), and the proof does not require the
dual spectral split or minimax. The argument above is retained as an
audited, unsuccessful attempt to remove the bounded-query-covariance
condition: replacing it by bounded operator norm of A merely returns
to a class already handled better by the affine construction.

The genuinely distinct bounded-covariance QUERY-law theorem remains
valid without bounded operator norm of A. A new uniform conclusion
would require weakening the A hypothesis, proving sufficient structure
for actual minimizing signings/dual laws, or obtaining a response value
below the scalar-affine scheme's established barrier. None is inferred
from the valid but redundant theorem above.
