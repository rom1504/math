...
## 8. Cloned-block preparation: stability at any prescribed shrinking scale

This section composes the director's exact-sign
[cloned-block regularizer](paper_director_cloned_block_regularization_2026_09_17.md)
with the all-energy argument above. The preparation is an ACTUAL same-order
full signing; the Gaussian neighborhood in this section is WEIGHTED, not
full sign. Section 6 of the director's artifact supplies the precise
integer-rounding and arbitrary-window construction.

There are universal constants c,C,eta_0>0 with the following property.
Let N be sufficiently large and

    N^(-2/3)<=eta<=eta_0,  ell=log(e/eta),
    p=floor(eta^(-1/2)),  r=floor(sqrt(N) eta^(3/4)),
    q=pr,  m=N-q,  B=N/sqrt(p).

Given ANY full signing A of order N, select the cloned realization W
as in that artifact. It can be selected once so that

    Q(W)<=Q(A)+C sqrt(eta) N^(3/2),
    b(E_W(D))<=D/r+C B for EVERY D>=0.                (21)

Here b is the full physical-spin Bernoulli width, not merely the core
projection width. Indeed the projection bound is D/r+c_r/r+Delta_r/r;
the q free cloned coordinates cost at most q. The assumptions imply
pr^2<=N, m>=N/2, log(r+1)<=C N/p, r+q<=C B, so all these terms are
absorbed by C B. Floors change only universal constants, including at
the endpoint r=1. No single W independent of eta is asserted.

Independently sample k standard Gaussian edge arrays G^1,...,G^k. If

    1<=k<=c N sqrt(eta) ell,
    0<=a<=c eta^(3/4)/sqrt(ell),
    T=C eta N^(3/2),                                 (22)

then with probability at least

    1-C exp[-c N sqrt(eta) ell],                      (23)

the ONE deterministic code E_W(T) contains every absolute optimizer of
W+sum theta_j G^j simultaneously for every ||theta||_2<=a. Moreover

    log|E_W(T)|<=C N eta^(1/4) ell,
    E sup_(||theta||<=a)|Q(W+sum theta_jG^j)-Q(W)|
          <=C eta N^(3/2).                           (24)

The preparation changes only edges incident to q<=sqrt(N)eta^(1/4)
vertices. If eta tends to zero, the preparation cost is o(N^(3/2))
and the witness has subexponential cardinality. The input may be an
exact minimizing signing, with no spectral assumption.

### All-energy proof and constants convention

Universal constants in (22) are chosen in the order: first the profile
constant in (21), then a sufficiently small amplitude constant c,
then a sufficiently large witness constant C. This order makes the
following finite inequalities hold; none depend on the original A.

Clipping and monotonicity, including the trivial width bound above the
range D/r+C B<N, give

    w(E_W(D))<=C sqrt(ell)(D/r+B) for EVERY D>=0.

The signed-edge comparison and the Gaussian product-index comparison
from Sections 2--3 therefore give

    E max_(d_i<=D)||(G^j dot(v_i-v_i0))_(j<=k)||
        <=A_0+D_0 D,
    A_0=C sqrt(N ell)B+2N sqrt(k),
    D_0=C sqrt(N ell)/r.                             (25)

The omitted two-polarity O(N) term is absorbed since B>=sqrt(N).
The random supremum is sqrt(2)N-Lipschitz. The assumptions in (22)
ensure aD_0<=1/16 and T>=16aA_0, after the constants are fixed as above.
On the shell 2^jT<=d_i<2^(j+1)T its mean times a is then at most
2^(j-2)T. Gaussian concentration and summation over every j>=0 give
the STRONGER simultaneous event

    |sum theta_j G^j dot(v_i-v_i0)|<=d_i/2
        for every d_i>=T and every ||theta||<=a.      (26)

The failure is at most C exp[-cT^2/(a^2N^2)], which is bounded by
(23) at the largest allowed a. Since
T^2/(a^2N^2)>=c N sqrt(eta)ell, even the smallest eta in (22)
has a diverging exponent. The same inner-level plus outer-shell
integration proves the expectation in (24). Thus no high-energy
complement was discarded. The case a=0 is deterministic.

Finally T/r<=C B, so (21), the VC/Sauer bound, and p~eta^(-1/2)
give the entropy in (24). More generally

    log|E_W(D)|<=C(ell/r)D for every D>=T.

Consequently the same half-deficit event (26) also captures the full
absolute partition function as in Section 7: for beta>=C ell/r,
the relative partition mass outside E_W(T) is at most
2 exp(-beta T/4), uniformly throughout the Gaussian ball.

For eta=N^(-gamma), 0<gamma<2/3, the scales are

    preparation cost       N^(3/2-gamma/2),
    witness/cap response   N^(3/2-gamma),
    log witness size       O(N^(1-gamma/4) log N),
    Gaussian dimension     k<=c N^(1-gamma/2) log N,
    coefficient radius     c N^(-3gamma/4)/sqrt(log N),
    failure exponent       c N^(1-gamma/2) log N.

This is uniform weighted-neighborhood stability at a selectable precision.
It is not a macroscopic full-sign extension, a fixed-eta asymptotic
recurrence, or a claim that every nearby sign matrix shares the witness.

The discrepancy researcher independently read and reconstructed this
entire Section 8 and returned PASS. The actual-sign random flip transfer
is proved separately in
[the noise-stability artifact](paper_localization_actual_sign_noise_stability_2026_09_17.md);
it must not be inferred merely by calling a Gaussian weighted matrix
a full signing.
