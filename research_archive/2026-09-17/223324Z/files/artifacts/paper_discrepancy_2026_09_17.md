...
Two subsequent standalone outcomes are
[the universal scalar-response entropy theorem](paper_discrepancy_scalar_response_entropy_2026_09_17.md)
and [the spectrally pinned full-sign application](paper_discrepancy_pinned_signings_2026_09_17.md).
The latter supplies a genuine bounded-cap family with zero-linear-slope
full-sign augmentation, while proving that its displayed certificate
cannot improve the supplied best child constant.
...
## 22. Actual finite minimizers: response duals without an isotropy constraint

The stored even-order minimizer representatives were also tested against
**every physical sign column**, not merely against isotropic column laws.
If a probability law `pi` on the absolute ground code satisfies
`min_(h in signs) E_pi |h dot x|>=beta`, then any full-sign extension
with `q` new vertices and new block `D` necessarily obeys

```math
Q(\mathrm{parent})\ge Q(A)+q\beta-Q(D).             \tag{45}
```

For each old word, align each new spin with its bridge overlap and the
old energy polarity, then bound the new-block term below by `-Q(D)`;
averaging over `pi` proves (45). There is no assumption on independence,
isotropy, or the bridge's construction.

The following lower certificates exhaust all sign queries. Except at
order ten, the displayed law is simply uniform on the projective
absolute ground code.

| Order | Old cap | Projective ground words | Certified beta |
|---:|---:|---:|---:|
| 4 | 4 | 2 | 0 |
| 6 | 5 | 12 | 5/3 |
| 8, either stored class | 10 | 8 | 3/2 |
| 10 | 13 | 40 | 10597/5000 |
| 12 | 18 | 20 | 9/5 |
| 14 | 21 | 156 | 98/39 |

At order ten the uniform certificate is `19/10`. A linear program
suggests optimum `53/25`; rounding its weights to denominator 10000
and exhaustively checking the resulting integer overlaps proves the
displayed lower value, without certifying the numerical optimum.
The zero at order four prevents a blanket finite-order assertion.

The global-minimizer labels are imported from the archived exact
certificates; caps, ground words, and all-column inequalities are
recomputed. These finite data do not prove an asymptotic ground-code
response bound. In particular, the values `5/3`, `9/5`, and `98/39`
coincide with the director's earlier isotropic mode-law calculations;
the additional content here is their pointwise ground-law certificate
without an isotropy restriction.

Replay:
`computations/paper_discrepancy_2026_09_17_ground_absolute_duals.py`.
The `--full` option prints all rational certificate atoms. The frozen
run is preserved under
`tmp/paper_portfolio_2026_09_17/discrepancy/ground_absolute_dual_certificates.json`.

## 23. Shifted absolute CLT and actual microscopic regularization

The separate self-contained
[shifted absolute-value theorem](paper_discrepancy_shifted_absolute_clt_2026_09_17.md)
proves, for q fair signs and every real shift t,

```math
\left|\mathbb E|q^{-1/2}\textstyle\sum_j\epsilon_j-t|
       -\mathbb E|G-t|\right|
\le\frac{\sqrt{2/\pi}}q
...
This is a Fourier central-lobe/lattice-tail proof, independent of the
Bernoulli track's Stein proof of the initially proposed `3/q` estimate.
It is NOT an order-`1/q` Wasserstein theorem. The numerical replay
exhausts all atom and inter-atom stationary extrema for q≤1000;
319,809 extrema were tested and the largest observed q-scaled error
was 0.20442252894471957 at q=3, t=0. The theorem does not rely on
that floating-point evidence.

The director uses this special two-slope comparison in the
[sparse random-star regularizer](paper_director_sparse_random_regularization_2026_09_17.md),
whose final proof the discrepancy track reconstructed in full. At every
large order N, rewriting O(N log N) edges suffices to preserve any input
cap within O(N sqrt(log N)) and force subexponential cardinality of the
complete nearcode in a physical window N/sqrt(log N). The construction
is entirely full-sign and includes all new-spin patterns and both
energy polarities. The regime q≈N^(1/3) gives a window and Gaussian
width O(N^(5/6)), and cap loss O(N^(7/6)). These are shrinking windows,
so they do not contradict the full fixed-η entropy floor above.

The separate
[extension-stability obstruction](paper_discrepancy_extension_obstruction_2026_09_17.md)
now also gives a concrete warning about this actual regularized class.
Apply a sparse-star extension to the explicit near-half Hadamard
family. The resulting small-window, small-width full signings W still
obey, for ANY further full-sign bridge with r≈εN new vertices and
child cap at most L r^(3/2),

```math
\liminf\frac{Q(\mathrm{parent})-Q(W)}{N^{3/2}}
\ge\frac{\varepsilon}{\sqrt{18}}-L\varepsilon^{3/2}.
```

Thus microscopic regularity alone does not imply an extension with
zero linear cost, even if future bridge columns may be arbitrarily
dependent and nonisotropic. These explicit examples have normalized
cap tending to 1/2; the obstruction is not asserted for the unknown
asymptotic minimizers.

Subsequently the director's
[exchangeable-sign information theorem](paper_director_exchangeable_sign_regularization_2026_09_17.md)
removed the restricted q range and proved, for arbitrary q≤N/2,
simultaneous cap cost O(N sqrt(q)) and conditional one-random-vertex
increment Δ(W)≤O(N/sqrt(q)). Every nearcode then has Bernoulli
width at most T+Δ(W), and VC/Sauer bounds its entropy. The discrepancy
track audited this full proof and independently replayed 4,772 finite
extensions and 692 nearcode inequalities in
`computations/paper_discrepancy_2026_09_17_exchangeable_information.py`.

The later
[codewise field-universality theorem](paper_director_codewise_field_universality_2026_09_17.md)
was also reconstructed in full with PASS. Combining exact new-spin
optimization, scalar field replacement, and the exchangeable
regularizer balances the preparation and restricted comparison errors
at O(N^(4/3)(log N)^(1/3)) on actual asymptotically minimizing full
signings. This improves a proven restricted comparison; it does not
bound the Gaussian parent value or exclude far old states.

The separate
[low-entropy value obstruction](paper_discrepancy_low_entropy_value_obstruction_2026_09_17.md)
isolates that issue. Polynomial-size codes can force the independent
absolute-overlap constant against every physical sign law; their
quadratic-feature covariance shows why those particular random codes
cannot themselves be positive-cap near-ground codes. A finite mean
sandwich reduces the remaining restricted value question to the
...
Appendix B fixes any prescribed proxy K>1 and gives the sharper capture
profile kappa[(1-1/K)sqrt(1-s)+(1/K)sqrt(1+(K-1)s)]. Appendix C preserves
arbitrary deterministic scalar offsets, uniformly even when the offsets
grow with the order. The resulting
[complete-child comparison on a declared old code](paper_bernoulli_feature_parent_comparison_2026_09_17.md)
retains every new-spin state and both absolute polarities: its finite
expected comparison cost is q sqrt(n) times the scalar error, plus
sqrt(4Kqn log(2|C|)). The comparator is the actual Gaussian variance
mixture, not an independent standard Gaussian. This is not a joint
scalar-to-maximum invariance assertion without a selection cost, nor a
bound on the mixture-parent value or the omitted old words.
...
## 26. Independent final-synthesis accuracy and source audit

At the director's request, the discrepancy track read the complete
[final synthesis](paper_portfolio_final_synthesis_2026_09_17.md) against
the canonical proofs and checked every displayed arXiv identifier against
its primary landing page. The main mathematical conclusions, limit
orders, exact physical support, covariance/SG distinctions, and retained
parent-value/escape obligations PASS. The following presentation fixes
were sent to the director before publication:

- The growing-rank statement fixes FINITELY MANY component variances.
  If e_n is defined only as the rate (r^2/n)^(1/4)+r/sqrt(n), the finite
  parent comparison has first term C_(L,v) q sqrt(n) e_n; coefficient one
  is valid only when e_n denotes the actual proved scalar-error bound.
- The finite two-sided block-information bound requires
  0<epsilon<1/4 and b>=4096 epsilon^(-2). The lower bound by itself uses
  b>=epsilon^(-2). The block-size premise must not be omitted.
- The smooth-central-energy exclusion assumes bounded normalized caps
  for its fixed list of full signings and a likelihood normalizer bounded
  below by a fixed positive m. Its error includes that normalization cost.
- The linked [Carlen--Cordero-Erausquin--Lieb paper](https://arxiv.org/abs/1106.0709)
  supplies the Brascamp--Lieb covariance inequality for log-concave
  measures. It should not be labeled as the separate Gaussian
  product/Hölder inequality used for Gaussian-sign concentration.

The GS links resolve respectively to Bansal--Dadush--Garg--Lovett and
Harshaw--Sävje--Spielman--Zhang; the localization/decomposition links to
El Alaoui--Montanari and Eldan; the Bernoulli-process, Grothendieck,
mixed-tail and convex-distance links resolve to the intended sources.
No external-priority claim was inferred from this bibliographic check.

The decisive final quantitative scope check is unchanged: at q=Theta(n),
the prepared log-cardinality bound gives a certified selection scale
n^(3/2-gamma/8)sqrt(log n), whose ratio to the protected window
n^(3/2-gamma) is n^(7gamma/8)sqrt(log n). This is failure of the available
upper certificate, not a lower bound on the actual fluctuation. Exact
child optimization removes a new-spin union, not the old-code payment
or the omitted branch-mean/escape obligation.

At 22:25 UTC the revised synthesis incorporated all four accuracy fixes
above: final read PASS. The added
[equal-variance offset tradeoff](paper_director_equal_variance_offset_tradeoff_2026_09_17.md)
was also read and reconstructed in full. The stop-loss integral equals
the second moment; a discount delta gives negative area at least
delta^2/2; the two subGaussian tails outside the stated R consume at
most delta^2/4; hence some offset in [-R,R] incurs loss at least
delta^2/(8R). The proof and synthesis correctly refrain from asserting
that an actual child produces this offset. This does not conflict with
physical-to-variance-mixture approximation.
