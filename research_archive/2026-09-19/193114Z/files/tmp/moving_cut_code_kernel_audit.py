#!/usr/bin/env python3
"""Finite audit of the rank-one moving-projection coset inequality.

This is scratch research code.  It reads only committed project artifacts and
prints a JSON table.  It deliberately writes no output files.
"""
...
def rank_one_kernel_data(length: int, level: int) -> tuple[float, np.ndarray]:
    ...
    # The paper uses w_i = sqrt(binomial(length, i)) v_i and block
    # amplitude squared w_i / sum_j w_j.
    log_weight = np.array(
        [
            0.5
            * (
...
def audit_one(n: int, histogram: dict[int, int]) -> dict:
    ...
    for level in range(1, min(length - 1, 3 * n) + 1):
    ...
    return {
        ...
        "levels_tested": [1, min(length - 1, 3 * n)],
...
def main() -> None:
    ...
    for n in (6, 8, 10, 12, 14):
        records.append(audit_one(n, projective_energy_histogram(witness_matrix(n))))
    ...
    print(json.dumps({"classification": "finite_numerical_audit", "records": records}, indent=2))
