...
The lower bound began with a plateauing search for a better upper bound. The system explored several variants of the limiting Krivine schemes. Although the constructions differed, they repeatedly encountered
...
Inspired by the works on mathematical problem-solving frameworks of~\cite{carlson2005problemsolving, schoenfeld1983episodes}, we present the following decomposition of long-horizon mathematical
research in the harness (see Figure~\ref{fig:research-cycle}). At any point in time, a research program has an \emph{active represented state}. this is a compact represenation of the complete history of the run for the purpose of choosing the next step, including current objectives, established and conjectural claims, evidence, failed approaches, open proof obligations, and unresolved choices. From this state, the
...
mathematical knowledge and problem-solving skills to develop lemmas and proofs, and it also effective ran and interpreted experiments. A prominent example of this was how the system found and developed the current lower bound, as detailed in the case study. After the upper-bound failures were reframed as evidence for a
...
should do next. These decisions combine mathematical valuation-- the
...
gained from failure, verification cost, and opportunity cost. Mathematical
value is itself multidimensional, rather than reducible to the numerical
strength of a bound~\cite{tao2007good,thurston1994proof}.
...
rather than asking whether the common obstruction was universal, and can be proven as a theorem. Other failings of the system included not recognizing the value of a theoretical result over numerical evidence, and prioritizing a sharper restricted result that is not inherently interesting over a general result. The relevant limitation was not an inability to perform the required
...
session~44 the uncertified score had set the record. A feasibility criterion
that would have invalidated the record fared worse, where it was proven by the run in
session~8 and lost in a handoff, and then it was reproven
from scratch by the audit that withdrew the record twenty-five days later. In both failures the archive retained
the original facts; it was the compressed state governing decisions that
failed.
...
experience~\cite{tao2007good,thurston1994proof}. The same holds for the research-state representation, where understanding what information to actively store that would be useful for future directions requires foresight and a background doing mathematical research.
