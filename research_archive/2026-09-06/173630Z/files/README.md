...
At every substantive checkpoint, inventory `tmp/` and other untracked or
ignored research files. Preserve new or changed research-bearing material in
the dated tracked [`research_archive/`](research_archive/README.md), including
unfinished derivations, exploratory scripts, failed experiments, counterexamples,
subagent reports, parameters/seeds, and informative outputs. Preservation does
not require polishing or verification: label drafts and unverified claims
explicitly, and do not promote their evidence status by archiving them.
Keep canonical proofs and certificates in their existing locations. Record
source paths, hashes, exclusions, and any missing dependencies in the archive
index/manifest; check that committed proofs and scripts do not silently rely
on ignored inputs. Commit and push the preservation checkpoint while research
continues. Report anything still uncommitted and why.
...
Temporary placement must never cause research knowledge to be lost. `tmp/`
remains ignored as a working directory, but its research-bearing contents must
be copied into a dated tracked research archive before cleanup and at every
substantive checkpoint. Do not delete working originals merely to make Git
status clean. Exclude environments, caches, credentials, downloaded software
dependencies, and reproducible build products—not exploratory research or
negative results. For research outputs too large for Git, preserve a tracked
manifest with checksums, provenance, reproduction instructions when available,
and an independently durable storage location; a local ignored path alone is
not durable preservation. If durable storage is unavailable, retain the
original and report the preservation gap rather than silently omitting it.
