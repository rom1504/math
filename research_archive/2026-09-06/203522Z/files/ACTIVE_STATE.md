...
Evidence cutoff: ledger Section 10.150.5, renewed campaign checkpoint 5.
...
The new upper is (1-2delta)^(-2)[1/2-(a+p[h(delta)+g])/(8sqrt(p))],
...
- Some actual near-minimizers exclude isotropy on every o(n^(4/3))-slack
  ...
  near-minimizers EXIST at every order: Q<=M_n+O(n^(11/8)), |P-R|=O(n^(11/8)).
  This balances scalar orientations, not covariance, and need not preserve op norm.
...
3. Actual-feedback transport: test small-amplitude signed covariance against
   literal coherent returns QS,QD. Marginal entropy alone does not determine it.
   No improvement of the old terminal functional is the present target.
