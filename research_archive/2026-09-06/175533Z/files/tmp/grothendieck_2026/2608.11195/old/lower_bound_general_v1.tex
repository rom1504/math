...
\title{The lower bound proof in general dimensions}
\author{}
% \date{}
...
\begin{abstract}
\noindent This is the general-dimensional version of the two-dimensional exposition. The proof is the same local argument, except the transverse coordinate $T$ is now a transverse Gaussian vector. We also include the final analytic transfer step which turns the affine strip into the conditional lower bound
\[
        K_G\ge \frac{6\pi}{11}.
\]
The only non-analytic input is the one-dimensional fiber inequality~\eqref{eq:fiber-inequality}.
\end{abstract}
...
then
\[
        \psi_\alpha(X)=\prod_{i=1}^d \psi_{\alpha_i}(X_i).
\]
Any square integrable function has a Hermite expansion, written as
\[
        F=P_0F+P_1F+P_2F+P_3F+\ldots,
\]
where $P_iF$ is the orthogonal projection of $F$ onto the span of all $\psi_\alpha$ with total degree $|\alpha|=i$.
...
Similarly, $P_3F$ is the projection onto the full total degree-three Hermite span. We will not list all basis elements. The important point is that it includes pure cubic directions such as $\psi_3(X_i)$ and mixed directions such as $\psi_2(X_i)\psi_1(X_j)$ and $\psi_1(X_i)\psi_1(X_j)\psi_1(X_\ell)$.
...
\section{Proof of the affine strip}
\label{sec:affine-strip}

\subsection{Agreement and disagreement}
...
\[
        ...
        \ge \ip{h}{\psi_3(S)}^2,
\]
because $\psi_3(S)$ is one orthonormal degree-three Hermite direction.

The trick we will use is to condition on the transverse variables, so define
\[
        A(s):=\E_T[h(s,T)].
\]
...
\subsection{The disagreement part}
...
Define the one-dimensional fibers
\[
        u_t(s):=k(s,t).
\]
Since $k\in\{-1,0,1\}$,
...
\]
Thus $K_j$ is the $j$th Hermite coefficient of the $S$-fiber, as a function of the transverse variable $t$.

In two dimensions, the four degree-three basis elements were
\[
        \psi_3(S),\qquad
...
\[
        P_3k
        =\sum_{j=0}^3 \psi_j(S)\,\Pi^T_{3-j}K_j(T),
\]
...
\begin{assumption}[Fiber inequality]
...
We treat~\eqref{eq:fiber-inequality} as the one computational input. The reason it is a one-dimensional numerical problem is that $V(u)$ depends only on the four moments $\E[u\psi_j]$, $0\le j\le3$. By the dual formula for the Euclidean norm,
\[
        \sqrt{V(u)}=\sup_{\norm{a}_2=1}\E[u(Z)p_a(Z)],
        \qquad
        p_a(Z)=\sum_{j=0}^3a_j\psi_j(Z).
\]
For fixed $p_a$ and a fixed support price $\tau$, the best ternary $u$ is the threshold rule
\[
        u(z)=\sgn(p_a(z))\one_{\{|p_a(z)|\ge \tau |z|\}}.
\]
Thus the infinite-dimensional search over $u$ reduces to a finite-dimensional search over a cubic Hermite direction $a$ and a threshold $\tau$. The active set is cut out by one-dimensional polynomial inequalities, so the required Gaussian integrals are over intervals. Those interval integrals are what the Arb certificate checks.
...
\section{The affine strip bounds the inverse majorant}
...
Let $H_k$ and $\gamma_k$ be supplied by Lemma~\ref{lem:inverse-majorant-optimal}. Sections~\ref{sec:preliminaries} and~\ref{sec:affine-strip} establish the affine strip for every pair of odd sign functions in every dimension. It therefore holds for every mixed scheme, because $b_1$ and $b_3$ average linearly. Passing to the coefficientwise limit shows that
\[
...
\begin{thebibliography}{NR14}

\bibitem[NR14]{naor2014krivine}
