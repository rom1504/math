...
For `q` distinct independently sampled tensor words `W_j` this implies
...
witness of a hollow matrix `L` on `N` vertices, put `p=n/N` and
`pi_j=(n)_j/(N)_j`. Its restricted signed energy `Z` has mean
