...
def main():
    ...
    a = {0: (1, -1), 4: (0, 1), 8: (1, 0)}
    assert t.evaluate(a, 11) == (0, 0)
    assert t.evaluate(f4, 11) == (4, 0)
    ...
    for k in [1, 2, 3]:
        ...
        for leaf in range(N):
            shift = k+6*leaf
            fixed_word |= 11 << shift
            if bin(leaf).count("1") % 2:
                fixed_word |= 1 << (shift+5)
        restricted = {}
        for mask, value in F4N.items():
            sign = (-1)**bin(mask & fixed_word).count("1")
            key = mask & (N-1)
            restricted[key] = t.add(restricted.get(key, (0, 0)),
                                    (sign*value[0], sign*value[1]))
        restricted = {i:z for i,z in restricted.items() if z != (0,0)}
        assert restricted == {N-1: (4*N, 0)}
