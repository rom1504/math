...
for name,indices in [('all',range(9)),('group1',[1,3,5,7]),('group2',[2,4,6,8])]:
    rows=[dict(label=f'm9_census{c["class"]}',matrix=c['matrix'],cap=12,
        source=str(census.relative_to(ROOT)),root_orbit_size=c['root_orbit_size']) for c in classes if c['class'] in indices]
