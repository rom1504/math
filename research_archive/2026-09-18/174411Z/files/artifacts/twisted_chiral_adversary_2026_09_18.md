...
The initial solve is in progress at this checkpoint. No infeasibility
claim is made while its status is `RUNNING` or `UNKNOWN`. Even an
`INFEASIBLE` CP-SAT response would be reported as solver-certified,
without a standalone proof object, and not as a lower bound over
all nonchiral order-sixteen signings.
