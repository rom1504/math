...
## All-temperature scalar endpoint audit

The added Section 7 also **PASS**es. With minority fraction r, write
v=4r(1-r), H=p h(r), J=h(pr)-H>=0, and
`d=(1-p)r/(1-pr)`. Revealing the minority label has information h(pr)
and residual variance exactly dv. The constant and minority labels give
`A_t>=max(H-tv,-d tv-J)`. For z=tv, the two resulting lower bounds on
`1+A_t/z` meet at z=(H+J)/(1-d). Their maximum is minimized there, yielding
`(1-d)H/(H+J)`. This proves the displayed all-temperature lower bound.
Since H/h(pr)->1 and d->0, its limit is 1/(2sqrt(p)). The reverse bound
follows by taking t to infinity at each fixed minority fraction.

An independent useful proof of the limiting statement is as follows.
The constant label gives coefficient/variance at least
`H/(2t sqrt(p)v)`, while the minority label gives at least
`[1-O_p(v)-O_p(1/t)]/(2sqrt(p))`. Since H/v grows like
`(p/4)log(1/v)`, split temperatures at sqrt(log(1/v)). Neither range can
beat the limiting 1/(2sqrt(p)) floor. This matches the stronger exact
two-branch calculation and is a certificate obstruction only, not a
lower bound for the original unscarred ensemble's actual bulk cap.
