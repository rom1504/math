...
## 9. Exact two-shore centering of the first feedback step

There is also a finite centering identity, provided sign(0)=0 is used
as a cube mean (or ties are symmetrically randomized). In any tree all
of whose vertices have odd degree, each shore contains an odd number of
vertices: the sum of degrees on that shore is the odd total edge count.
After deleting an external root on the left, an old tree monomial has
an even number of left source spins and an odd number of right source
spins. Its character under the two global source-shore flips is (+,-).
For a right output root the character is (-,+).

Consequently any odd local old response F_L has character (+,-), and
F_R has character (-,+). An even local hole mask H is invariant under
both flips. Since B crosses shores, T=H sign(BF) has the reverse
characters. Thus H_(A_0)(F) and H_(A_0)(T), where A_0=sqrt(m)B,
both have character (-,-), and their expectations are exactly zero.
The cross term is unchanged, so the SINGLE feasible mean F+T satisfies

    E H_(A_0)(F+T)=sqrt(m) sum_i E H_i |(BF)_i|.

This independently checks the cap factor without comparing two means.
The finite sign convention sign(0)=+1 would spoil exact parity on ties;
an odd soft sign or the convention above removes that issue. A later
response based on the full F+T has mixed characters, so this identity
does not by itself prove a higher-pass closure or a new numerical gain.
