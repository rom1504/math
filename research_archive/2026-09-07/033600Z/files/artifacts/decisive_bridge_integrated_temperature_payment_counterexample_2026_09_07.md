...
## 5. Sharper version inside the available optimizer cap budget

The same construction has a stronger parameter choice. Let

    k=2^(2r+1), ell=4k, N=k ell, ell/sqrt(N)=2,

and use the same S,B,A. Split the outer Sylvester indices by their first
bit, so the two outer principal blocks are +H_(k/2) and -H_(k/2), each
with its diagonal removed. Both k/2 and ell/2 are powers of four.

Every Sylvester Hadamard H_(4^j), j>=1, has flat Boolean eigenvectors
with eigenvalues +sqrt(4^j) and -sqrt(4^j). For H_4 use respectively
(1,1,1,-1) and (-1,1,1,1), then take tensor products. Since tr H_d=0
for these orders, removing the diagonal does not alter their quadratic
energies. In each child of order d=N/2, choose an outer flat vector s
with s^T S_child s=(k/2)sqrt(k/2), and a flat inner vector
z=(v,-v) with Bz=-sqrt(2 ell)z. The Boolean product s tensor z has
zero clique means and yields the exact negative-energy witness

    H_child(s tensor z)=-d/2-sqrt(2)d^(3/2)/2.         (7)

Combining it with the positive coherent configuration, and using the
two ground configurations as lower bounds for the two branch partitions,
gives the stronger lower bound

    sum_children L_child(beta)
        >= (beta N/4)(2sqrt(2)+sqrt(2)).              (8)

The positive diagonal corrections in the negative witness cancel the
negative diagonal corrections in the positive witness exactly. Meanwhile
the same parent upper bound is

    L_A(beta) <= Nlog2+(beta N/4)(2+Loff/sqrt(N)).

Thus at beta=4,

    G_A-T_A <= N[log2-2(sqrt(2)-1)+sqrt(2/k)]
              =(-0.1352799440...+o(1))N.              (9)

For sufficiently large k the parent's exact positive maximum exceeds
the upper bound (3) for its negative maximum. Hence Q(A)/N^(3/2)
tends to 1 (and Q(A)<=N^(3/2) eventually). In particular the scaled
interaction J=4A/sqrt(N) obeys Q(J)<=4N eventually. This is below
the width-optimizer a priori cap budget C_4 N, where even the random
cap candidate gives C_4=2log2+8sqrt(log2)>4. It is also below the
budget obtained by inserting the improved all-order cap upper constant
0.494515125: 2log2+8(0.494515125)>4.

This sharpening does not make the construction a global optimizer.
It shows that merely inserting the presently available numerical
optimizer cap budget into the full Hubbard identities is still
insufficient. Some genuinely additional consequence of global
sign optimality must be used to establish integrated payment.
