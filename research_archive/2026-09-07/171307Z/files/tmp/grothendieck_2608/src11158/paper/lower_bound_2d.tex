...
\begin{enumerate}
    \item We show a lower bound.
    \item The key idea is to use Naor-Regev lower bound saying Krivine schemes are optimal. By reworking their proof, we extract an analytic form in terms of majorant series and we use that.
    \item Finally, a technical lemma we show is that the first, third Taylor-series coefficients of $H_{f,g}$ for arbitrary odd $f,g$. 
    \item We then show that this linear inqequality implies control on the inverse majorant, leading to the lower bound. 
\end{enumerate}
...
The last step of the proof (Section~\ref{sec:strip_to_KG}) has two parts.
\begin{itemize}
    \item The affine strip forces every admissible inverse-majorant value to satisfy $\gamma\le11/12$.  Because the strip is affine, the same barrier holds for mixtures.
    \item The construction in the proof of Naor--Regev's optimality theorem shows that the inverse-majorant method itself is asymptotically optimal: it produces admissible values converging to $\pi/(2K_G)$.  The barrier therefore gives $K_G\ge6\pi/11$.
\end{itemize}
...
\label{sec:strip_to_KG}

Once the affine strip has been established in every dimension, the lower bound follows in two steps.  First, in Section~\ref{sec:strip_majorant} we show that the strip gives a universal barrier for the inverse-majorant parameter, and this barrier is preserved under mixing. Second, in Section~\ref{sec:majroant_optimal} we note that the construction in the proof of Naor and Regev's optimality theorem shows that the inverse-majorant method for proving upper bounds on $K_G$ itself is asymptotically optimal.  Combining these two facts gives the lower bound on $K_G$.

We begin by recording what we mean by mixing rounding rules.
...
The coefficients $b_1$ and $b_3$ depend linearly on the correlation function.  Hence the affine strip is preserved under mixtures and coefficientwise limits.  We use this in the proof of Theorem~\ref{thm:lower-bound}, where the strip is transferred to the limiting mixed functions supplied by Lemma~\ref{lem:inverse-majorant-optimal}.

\subsection{The affine strip bounds the inverse majorant}
\label{sec:strip_majorant}

The inverse-majorant parameter is a property of the correlation function itself, so we define it for any odd analytic function.

\begin{definition}[Admissible majorant value]
...
This is the usual inverse-majorant parameter in Krivine's upper-bound method.  For an actual mixed scheme, the standard tensor preprocessing argument turns an admissible value $\gamma$ into the upper bound $K_G\le\pi/(2\gamma)$.  We will not use that implication directly; it only explains the role of $\gamma$.
...
\subsection{The inverse-majorant method is optimal}
\label{sec:majroant_optimal}

The previous proposition is initially only a limitation of one upper-bound method. The next lemma is what transfers this limitation into a lower bound for $K_G$: the construction used in the proof of Naor and Regev's theorem produces inverse-majorant values that asymptotically attain the best possible value.

\begin{lemma}[Optimality of the inverse-majorant method]\label{lem:inverse-majorant-optimal}
...
\]
Equivalently, the inverse-majorant upper bounds satisfy
\[
        \frac{\pi}{2\gamma_k}\longrightarrow K_G.
\]
Thus, after allowing mixtures and increasing dimension, the inverse-majorant method is asymptotically optimal.
\end{lemma}
...
\begin{proof}
Let $H_k$ and $\gamma_k$ be supplied by Lemma~\ref{lem:inverse-majorant-optimal}.  As explained in Section~8, the affine strip holds for every rounding pair in every dimension.  It therefore holds for every mixed scheme, because $b_1$ and $b_3$ average linearly.  Passing to the coefficientwise limit shows that
\[
