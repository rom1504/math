...
The later complete seed-pressure theorem removes the tail restriction
entirely: ALL per-row bounded-energy profiles have the same leading fixed-`t`
port pressure for low-cap seeds, including coherent spikes. Its aggregate
version has the sharp seed threshold `Q=o(m^2/log m)`. See
`principle_synthesis_2026_09_07_complete_seed_pressure_universality.md` and
`principle_director_sharp_port_universality_2026_09_07.md`. Thus neither
signed asymmetry nor coherent tails rescue a leading gap in this particular
common independent-port pressure. Minimum-defect universality and the
energy-scaled diverging-temperature comparison are also proved there.

The exact finite responses in this note are still valid: the theorem
removes their proposed LEADING large-order implication, not their finite
or law-level distinction. Different seed-dependent profile laws,
nonproduct port choices, an unscaled critical-temperature effect, or a
different constant-multiplier operation remain outside that comparison.
The most direct independent two-row constant-multiplier law has now been
tested separately at the actual parent level and has cap above `.50717`;
correlated constant-size frames are not excluded by that scoped result.
