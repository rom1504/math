...
## Addendum: zero temperature and finite-accuracy entropy

The canonical Sections 9--11 were subsequently read completely and also
**PASS**. For bounded profiles, fixed-grid quantization has an exact uniform
image law, with at most `L_grid^(m(m-1))` equally likely arrangements.
Therefore its soft-hard gap in energy units is at most
`m(m-1) log(L_grid)/t`. Fixed-temperature pressure universality, followed
by t tending to infinity and then grid refinement, proves uniform minimum
port-defect universality for bounded profiles.

The matched-heavy construction also works deterministically: start at a
minimizing light arrangement, move tagged zeros using the same bounded
number of swaps, and insert compatible heavy categories. The category
probability is omitted, leaving exactly (23). Annulus deletion then proves
the zero-temperature result for arbitrary row-energy-bounded profiles.

Dividing the complete soft-tail bound (17) by t leaves only fixed-accuracy
entropy constants divided by t. Combining this with the bounded-grid
soft-hard estimate proves the uniform Laplace principle for EVERY
diverging temperature sequence. There is no need for t/log(m) to diverge.
That stronger requirement belongs only to the naive full-permutation
entropy estimate, which this compiler improves. The resulting pressure
comparison is `o(t_m m^2)`, not unscaled `o(m^2)`.

The finite-accuracy probability consequence follows with a sufficiently
large but fixed t, selected after epsilon and before m. If the soft-hard
gap is at most epsilon m^2/2, splitting the Laplace expectation at defect
`G_S+epsilon m^2` yields probability at least
`exp(-t epsilon m^2/2)-exp(-t epsilon m^2)`, hence at least
`exp(-A_epsilon m^2)` for large m. This requires no exchange of an
instance-dependent minimizer with the simultaneous maximum over spins.
