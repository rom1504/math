...
is false in the smallest possible example: one query and the one-state
max-plus family `f_v(*)=v`, `v in [-B,B]`, have fat dimension one at every
scale below `B`, but covering entropy `Theta(log(B/epsilon))`. The missing
factor is scalar precision. The corrected law below retains it explicitly.
...
```math
\boxed{
\log_2\operatorname{Cov}_{r+\eta/2}(\mathcal F,d_{\rm sh})
...
The nonprojective companion is
...
```math
\log_2\operatorname{Cov}_{Lr+\eta/2}(\mathcal F,\|\cdot\|_\infty)
...
Thus exposed queries form a `2gamma`-packing. If `C` is any such packing,
...
with the left inequality interpreted by the usual volume bound. When all
`p` coordinates are exposed this is the sharp
`Theta(p log(B/epsilon))` law. Query landmarks give the complementary upper
...
=O_\epsilon(m^2+m\log(w+m)).                     \tag{MC.3}
```
