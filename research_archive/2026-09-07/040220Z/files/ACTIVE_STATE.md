...
Evidence cutoff: ledger Section 10.151.6, sixth decisive-limit checkpoint.
...
The [fresh standalone reconstruction](artifacts/decisive_audit_standalone_direct_E_upper_2026_09_07.md)
shortens the necessary chain: BE<=E and direct stopping suffice; the equality
H=E and its lower conditional-copy construction are not upper-bound premises.
The full 212505-rectangle certificate was independently replayed again.
...
Sixth checkpoint: the marked proof now independently extends to complete
square bipartite full-sign support, giving
`liminf min_C ||C||_(infinity->1)/m^(3/2)>=.8666442233281615`.
This is not an arbitrary weighted-matrix theorem. A new actual same-order
sign surgery turns a low-rank quadratic RAMP certificate for a dominant
energy surplus into midpoint repair: error O(n^(4/3)r^(1/3)) plus the
certificate error when r=o(sqrt(n)). An incoherent rank-o(n) variant also
works. The ramp is realizable in a planted-bias signing family, but has NOT
been proved for width minimizers. The stronger all-high-state capture
hypothesis is circular and was rejected. Neither result proves convergence.

Fifth checkpoint: the lower certificate has now been reconstructed from its
marked-response dependencies and replayed byte-for-byte; see
`artifacts/decisive_audit_fresh_full_lower_chain_2026_09_07.md`.
The exact log-cosh interpolation for actual weighted width minimizers splits
into a favorable PSD term and a signed imbalance of nonnegative edge-flip
costs. No sublinear integrated imbalance is proved, and width-to-absolute
transfer is a second gap. A scalable bounded-cap counterexample defeats
generic temperature payment, not optimizer-specific payment. Continuous
spectral regularization preserves leading outer entropy, but no all-order
entropy limit is known. Sparse triangular-array amplitudes rigorously refute
two-sided adaptive variance universality, not favorable Gaussian-to-flat
comparison. These statements do not change the original interval or settle
convergence. Work continues through the authorized endpoint.

Fourth-checkpoint independent progress: a bounded actual cap Q(J)=O(N)
implies a dimension-free lower covariance bound for delocalized quadratic
observables throughout a fixed bridge tilt. This yields a genuine positive
order-N gain in width pressure at contracted child temperatures, not the
same-temperature recurrence needed for convergence. The new continuous
soft-flatness approximation has uniform error <=log2/(4tau), but its
all-order variational limit is unproved. Neither result solves the original
liminf-to-limsup gap. See ledger 10.151.4 for scope and replay evidence.
