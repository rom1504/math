    ...
    for d in range(1, max(degrees) + 1):
        if d in degrees:
            ...
            reports.append({
                ...
                "exact_sum_squared_value_numerators": str(sum(v*v for v in f_current)),
                "numeric_L2": mp.nstr(mp.sqrt(mp.mpf(sum(v*v for v in f_current))
                                               / len(energy))/scale, 30),
                "exact_L2_exceeds_one": sum(v*v for v in f_current)
                                             > len(energy)*scale*scale,
