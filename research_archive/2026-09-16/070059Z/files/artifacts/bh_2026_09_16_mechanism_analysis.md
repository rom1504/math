...
The final saturation argument in Sections 10.3--10.4 strengthens this
scope assessment: in the unmasked actual model, every two-level core
choice either lies in an already small-cap variance range or gives only
a vanishing leading Schur improvement.
...
### 10.3 Every partial Hadamard frame saturates on sufficiently large cores

There is a simpler exact obstruction at positive column density, valid
for EVERY full frame and every selector, not only the special Sylvester
frame in Section 10.2. Let F consist of q rows of a sign Hadamard
matrix of order m, dephased so a marked column is the all-one vector.
Thus `F F^T=m I_q`. Delete that marked column, let C be any set of s
remaining active columns, and put `P=I-J/q`. If `s>m-q`, then

```
||F(:,C)||op^2=||P F(:,C)||op^2=m.                   (10.3a)
```

In fact eigenvalue m of the centered row Gram has multiplicity at
least `q+s-m`. The full complementary column set has m-s members,
including the marked constant column. Its transpose has kernel of
dimension at least q+s-m. Every vector u in that kernel is centered,
and full row orthogonality gives
...
This proves the lower singular bound; restriction and orthogonal
projection give the matching upper bound sqrt(m). The strict threshold
is sufficient; at equality the dimension count forces no positive
eigenspace.

These are the actual raw-frame hypotheses of the balanced construction:
the selected full transform is Hadamard, its marked DC column is the
self-port, and the repair estimate in the sparse-active artifact,
Section 6.3(12), is
`||T_i-P_i F_i||op<=epsilon_m sqrt(m)`, epsilon_m->0.
Thus on each eligible core the repaired squared singular norm is at
least `(1-epsilon_m)^2 m=m-o(m)`, while its global upper bound is
`(1+epsilon_m)^2 m=m+o(m)`. A physical core containing fibre i loses
its self column; additional masks can remove more columns. The exact
condition always concerns the number of SURVIVING active columns.

For unmasked cores of any fixed density delta>1-p, the condition holds
at every fibre eventually. In the actual p=31/32 model this includes
every fixed delta>1/32. Therefore the two-level port certificate cannot
gain a fixed leading amount there from a uniform restricted operator
bound: raw K=M=m, and repaired M-K=o(m). Its Schur improvement is at
most `(M-K)V_C/2=o(m V)`. This is an obstruction to that Euclidean
column-profile input, NOT a Boolean energy lower witness and NOT an
obstruction to all jointly paid port/packet mechanisms.

A targeted archive search for partial-Hadamard core saturation and the
complementary-column dimension argument found no prior statement of
this exact lemma. It is elementary linear algebra, not a claim of
external novelty. Independent proof-verifier check: PASS.

The exact rational replay
`computations/bh_mechanism_2026_09_16_frame_saturation.py` verifies 81
selected-frame/core instances at orders 4,8,16,32, including the full
complementary kernel, centered eigenvectors, and forced multiplicity.
Its canonical output is
`computations/results/bh_mechanism_2026_09_16_frame_saturation_replay.json`.
All checks and `py_compile` PASS. This is not a test of Boolean cap
attainment or of the asymptotic repair theorem.

### 10.4 Complete two-level Schur scope for the unmasked actual model

The root agent observed a stronger consequence of (10.3a); the algebra
below checks the exact finite thresholds, including self-column loss.
Assume the actual UNMASKED repaired model: every nonself column is a
balanced sign vector with squared norm q, self columns vanish, and
`max_i ||T_i-P_i F_i||op<=epsilon sqrt(m)`, with `0<=epsilon<1`.
Restrict now to physical Boolean words, so every fibre variance is at
most q and V<=N=mq. Set

```
p_m=q/m,          M=(1+epsilon)^2 m,
rho=V/(mq),       K_C=max_i ||T_i(:,C)||op^2,
b_m=(1+epsilon)^2(1-p_m+1/m)/p_m.
```

For any nonempty physical core C, some nonself row sees one of its
full columns, so `K_C>=q`. A strictly positive Schur improvement in
(8) requires `theta=V_C/V>K_C/M>=p_m/(1+epsilon)^2`. Since
`V_C<=|C|q`, this forces

```
|C|/m >rho p_m/(1+epsilon)^2.                        (10.4a)
```

If `rho>b_m`, then (10.4a) gives `|C|-1>m-q`. Every row has at
least |C|-1 surviving columns into C, so (10.3a) and repair imply
`K_C>=(1-epsilon)^2m`. Thus every core, including cores chosen after
seeing the physical word, has Schur improvement at most

```
(M-K_C)V_C/2 <=2 epsilon m V=o(mV).                  (10.4b)
```

Cores with zero improvement and the empty core satisfy the conclusion
without using (10.4a). On the other hand, if `rho<=b_m`, the existing
global estimate alone gives

```
|H_W|/N^(3/2)
 <=(1+epsilon)^4(1-p_m+1/m)/(2p_m^(3/2))
 ->(1-p)/(2p^(3/2)).                                (10.4c)
```

For p=31/32 the last constant is `0.016387113161314904...`.
Consequently, as epsilon->0 and p_m->31/32, every variance profile
falls either in an already tiny-cap range or in a range where this
two-level restricted-operator Schur certificate gains only o(N^(3/2)).
The conclusion is uniform over core choice and outer seed. It does
NOT cover arbitrary additional masks, which can destroy both the
full-column norm floor and the active-column count. Nor does it rule
out Boolean-specific restrictions, selector-law information, or other
joint port/packet inequalities. This closes the value-relevant scope
of the particular two-level extension, not all weighted mechanisms.
The independent proof-verifier audit of the exact finite thresholds
and both branches is PASS.
