...
from math import ceil, comb, exp, factorial, floor, sqrt
...
stdout_context = contextlib.redirect_stdout(transcript) if json_mode else contextlib.nullcontext()
...
if json_mode:
    result = {
        "schema": "bh-independent-proof-audit-v1",
        "source": "https://arxiv.org/html/2609.12427v1",
        "date": "2026-09-16",
        "method": "Exact rational finite checks plus analytic Hoeffding tail; exhaustive H6 enumeration",
        "all_checks_passed": all(item["passed"] for item in records),
        "checks": records,
        "tail_taylor_lower_bound": str(taylor),
        "original_contraction": original,
        "refined_contraction": c4,
        "refined_tail_contraction": tail,
        "refined_reciprocal_gap": 1 / (1 - c4),
        "weighted_degree_exponent": 3,
        "weighted_denominator_exponent": 5,
        "bh_degree_exponent": 5.5,
        "level_flat_influence_degree_exponent": 11,
        "h6_histogram": dict(sorted(energies.items())),
        "h6_B2_lower_bound": 15**0.75 / 5,
        "direct_B2_asymptotic_ceiling": 5 / 30**0.75,
