...
Evidence cutoff: bounded BH-paper investigation started2026-09-16.
Status: ACTIVE only for05:13:45--07:13:45UTC. Original convergence remains OPEN.
...
parameters and outputs throughout. Temporary placement must not lose knowledge.
Never use /tmp; use /home/math/quadra/tmp. Canonical proofs stay in place;
dated research archives preserve other research-bearing files. Exclude reviewed
...
Earlier authorizations and detailed route history remain in Git and ledger.
...
BH-paper checkpoint,2026-09-16: the primary proof reconstructs correctly.
The new polynomial degree loss does not remove the old signed-moment
obligation. Exact power/entropy identities and a stronger arbitrary-filter
audit now separate sublinear, linear and superlinear degree regimes.
Every growing sublinear scalar filter passes the current polynomial-BH
tests on bounded-operator full-sign controls, despite false cap hypotheses.
No numerical frontier, original recurrence or convergence result changed.
A weighted-port extension is valid but covers only already-negligible
total variance. Its relative improvement is not leading-order progress.
The bounded investigation continues until07:13:45UTC; a Boolean-Bohr-radius
connection and all new quantifiers are receiving independent verification.

The following is the preserved prior construction assessment, not a claim
that the new paper has advanced its missing dense-profile obligation.
