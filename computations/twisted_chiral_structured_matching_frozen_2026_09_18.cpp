...
void emit(const std::string& kind, const std::vector<int>& p,
  ...
  std::cout<<"]";
  if(kind=="complete_family"||kind=="complete_structured_subfamily"){
...
int main(int argc,char**argv) {
  ...
  if(mode=="matching") {
    std::unordered_set<uint64_t> distinct_b;
    std::vector<std::pair<int,int>> pairs;
    std::function<void()> recurse=[&](){
      int i=0;while(i<r&&p[i]>=0)i++;
      if(i==r){
        int orient_count=1<<(int(pairs.size())-(r%2==0?1:0));
        for(int mask=0;mask<orient_count;mask++){
          std::fill(s.begin(),s.end(),1);
          for(int k=0;k<int(pairs.size());k++){
            int bit=k-(r%2==0?1:0),sign=(bit>=0&&(mask>>bit&1))?-1:1;
            s[pairs[k].first]=sign;s[pairs[k].second]=-sign;
          }
          count++;uint64_t code=full_code(p,s);
          if(!distinct_b.insert(code).second)continue;
          auto e=evaluate(p,s);cap_hist[e.cap]++;width_hist[last_width]++;consider(e);
        }
        return;
      }
      for(int j=i+1;j<r;j++)if(p[j]<0){
        p[i]=j;p[j]=i;pairs.emplace_back(i,j);recurse();pairs.pop_back();p[i]=p[j]=-1;
      }
    };
    if(r%2){for(int fixed=0;fixed<r;fixed++){std::fill(p.begin(),p.end(),-1);p[fixed]=fixed;recurse();}}
    else{std::fill(p.begin(),p.end(),-1);recurse();}
    full_B_count=distinct_b.size();emit("complete_structured_subfamily",bp,bs,best,count,elapsed());
  } else if(mode=="quotient"||mode=="bound") {
