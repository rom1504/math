...
for n in range(2, 9):
    ...
    for trial in range(3):
        ...
        active_cross = cross[np.ix_(energy == energy.max(), energy == energy.min())]
        derivative = 2*k*np.abs(active_cross).max()
        small_angle = 1e-6
        small_values = k*math.cos(2*small_angle)*(energy[:, None]-energy[None, :])
        small_values = small_values+k*math.sin(2*small_angle)*cross
        numerical_derivative = (np.abs(small_values).max()-initial)/small_angle
        assert abs(numerical_derivative-derivative) <= 3*initial*small_angle+1e-7
