...
bool feasible_matching(const std::vector<int>& p,const std::vector<int>& s,int target) {
  ...
  for(int xx=0;xx<N;xx++) {
    int x=extreme_x_order[xx];
    int field[12]={},bilinear=0;
    for(int i=0;i<r;i++)for(int j=0;j<r;j++)field[j]+=spins[x][i]*B[i][j];
    int y_start=0;bilinear=field[0];
    for(int j=1;j<r;j++){if(field[j]<0)y_start|=1<<(j-1);bilinear+=std::abs(field[j]);}
    for(int k=0;k<N;k++) {
      ++bound_pairs;int y=gray[k]^y_start;if(k){int j=flipped[k];bilinear+=2*spins[y][j]*field[j];}
...
bool feasible_radius(const std::vector<int>& p,const std::vector<int>& s,int target) {
  ++bound_calls;
  for(int i=0;i<r;i++)for(int j=0;j<r;j++)B[i][j]=s[i]*s[j]*A[p[i]][p[j]];
  std::fill(lo,lo+N,10000);std::fill(hi,hi+N,-10000);
  for(int xx=0;xx<N;xx++){
    int x=extreme_x_order[xx],field[12]={},bilinear=0;
    for(int i=0;i<r;i++)for(int j=0;j<r;j++)field[j]+=spins[x][i]*B[i][j];
    int y_start=0;bilinear=field[0];
    for(int j=1;j<r;j++){if(field[j]<0)y_start|=1<<(j-1);bilinear+=std::abs(field[j]);}
    for(int k=0;k<N;k++){
      ++bound_pairs;int y=gray[k]^y_start;if(k){int j=flipped[k];bilinear+=2*spins[y][j]*field[j];}
      int t=x^y,e=qa[x]-qa[y]+bilinear;lo[t]=std::min(lo[t],e);hi[t]=std::max(hi[t],e);
      if(hi[t]-lo[t]>2*target)return false;
    }
  }
  return true;
}
...
Eval evaluate(const std::vector<int>& p, const std::vector<int>& s) {
  ++calls; profile(p,s);
  last_width=0;for(int t=0;t<N;t++)last_width=std::max(last_width,(hi[t]-lo[t])/2);
...
void emit(const std::string& kind, const std::vector<int>& p,
  ...
  std::cout<<"]";
  if(width_mode)std::cout<<",\"profile_radius\":"<<best_width;
  if(kind=="complete_family"||kind=="complete_width_family"){
    ...
    std::cout<<"}";
    if(!bound_mode){
      std::cout<<",\"unconstrained_profile_center_radius_histogram\":{";
      bool first_width=true;for(int k=0;k<1000;k++)if(width_hist[k]){
        std::cout<<(first_width?"":",")<<"\""<<k<<"\":"<<width_hist[k];first_width=false;
      }
      std::cout<<"}";
    }
  }
...
int main(int argc,char**argv) {
  ...
  std::iota(extreme_x_order,extreme_x_order+N,0);
  std::stable_sort(extreme_x_order,extreme_x_order+N,[](int a,int b){return qa[a]>qa[b];});
  ...
  auto consider=[&](const Eval& e){bool improve=width_mode?(last_width<best_width):(e.cap<best.cap||(e.cap==best.cap&&e.score<best.score));if(improve){best_width=last_width;best=e;bp=p;bs=s;emit("best",bp,bs,best,count,elapsed());}};
  if(mode=="quotient"||mode=="bound"||mode=="width") {
    width_mode=mode=="width";
    bound_mode=mode=="bound"||width_mode;
    ...
    for(const auto& root:roots) {
      ...
      for(int mask=0;mask<N;mask++) {
        ...
        if(width_mode){if(!feasible_radius(p,s,best_width-2))continue;}
        else if(bound_mode&&!feasible_matching(p,s,best.cap-2))continue;
        auto e=evaluate(p,s);cap_hist[e.cap]+=weight;width_hist[last_width]+=weight;consider(e);
      }
    }
    if(weight_sum!=full_B_count){std::cerr<<"orbit weight mismatch "<<weight_sum<<" != "<<full_B_count<<"\n";return 5;}
    emit(width_mode?"complete_width_family":"complete_family",bp,bs,best,count,elapsed());
    ...
    do {
      for(int mask=0;mask<N;mask++) {
        ...
        count++;auto e=evaluate(p,s);cap_hist[e.cap]++;width_hist[last_width]++;consider(e);
      }
    } while(std::next_permutation(p.begin(),p.end()));
