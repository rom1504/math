// Broader child-Q<=23 continuation. Original one-hour source remains frozen.
// Input adds the saved p and s after A; histogram capacity covers every order20 cap.
...
int main(int argc,char**argv) {
  const double seconds=argc>1?std::stod(argv[1]):1200;
  const uint64_t seed=argc>2?std::stoull(argv[2]):2026091903;
  const int child_limit=argc>3?std::stoi(argv[3]):23;
  ...
  std::vector<int> p(r),s(r,1);
  for(int i=0;i<r;i++)if(!(std::cin>>p[i]))return 4;
  for(int i=0;i<r;i++)if(!(std::cin>>s[i])||std::abs(s[i])!=1)return 5;
  auto sorted_p=p;std::sort(sorted_p.begin(),sorted_p.end());
  for(int i=0;i<r;i++)if(sorted_p[i]!=i)return 6;
  int initial_sign=s[0];for(int i=0;i<r;i++)s[i]*=initial_sign;
  ...
  std::array<uint64_t,256> evaluated_caps{},child_caps{};
  ...
  bool first=true;for(int k=0;k<256;k++)if(evaluated_caps[k]){std::cout<<(first?"":",")<<"\""<<k<<"\":"<<evaluated_caps[k];first=false;}
  std::cout<<"},\"evaluated_child_cap_histogram\":{";first=true;
  for(int k=0;k<256;k++)if(child_caps[k]){std::cout<<(first?"":",")<<"\""<<k<<"\":"<<child_caps[k];first=false;}
