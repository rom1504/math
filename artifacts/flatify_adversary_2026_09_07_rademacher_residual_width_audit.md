...
Subsequent strengthening: `flatify_adversary_2026_09_07_spectral_fourth_moment_audit.md` improves the covariance Frobenius bound to O(sqrt(n)), extending the same half-width theorem from rank o(sqrt(n)) to EVERY rank o(n). The text below preserves the first audit of the weaker preliminary version.
