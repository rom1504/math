...
At m=4, N=32, one has the stronger universal upper **Q(A)<=86**, giving
86/32^(3/2)=.475087369... . First, for any Hadamard S of order 32 and any
...
Thus |x^T Sx|<=176. Symmetry excludes equality. If equality held, replace
S by its switching with x and, if needed, its negative. Equality in the
moment bound and in sum w_i<=sum |w_i| forces its row-sum vector r to have
28 coordinates equal to six and four equal to two. Write

    r=6*1-4*1_T,      |T|=4.

Since S1=r and S^2=32I, one has Sr=32*1, hence

    S1_T=(6r-32*1)/4.

This equals -5 on T, impossible because every entry of S1_T is a sum of
four signs. Finally x^T Sx is a multiple of four when tr(S)=0, because the
hollow edge count 496 is even. It is consequently at most 172 in modulus,
proving Q(A)<=86. Any explicit witness with energy 86 is therefore an exact
cap certificate without an exponential search. In particular this applies
to the constructive track's saved randomized order-32 rank-two outputs.

This discrete improvement vanishes in the general large-order estimate
and is not a proof of an asymptotic constant below one half.
...
In fact the same embedding works for any FIXED finite r-fibre frame bundle,
not just common Walsh frames: set the enlarged frames
F_(p,i)=H_L tensor F_i. Their resulting matrix is R_L tensor S_small after
reindexing. Thus every finite bundle witness propagates through its own
tensor-compatible family, with exactly the same L^3 energy multiplication.
