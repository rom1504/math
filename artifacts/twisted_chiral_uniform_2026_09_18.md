...
uniform-theorem track, resumed September19 with audited additions through
the diamond theorem and native-selection audit below. Convention throughout:
...
O(n) entries of bounded magnitude. The primary
[Schmidt theorem, Theorem1 and its Fourier normalization](https://math.uni-paderborn.de/fileadmin-eim/mathematik/AG-Diskrete_Mathematik/Publications-schmidt/pw.pdf)
and the same-spin embedding audited in
...
accounting are proved. The main positive joint-norm result is the sharp
`Delta(A)<=4beta(A)/3`, alongside `Delta(A)<=2W(A)+beta(A)/2`. It gives
an explicit balanced-clone lift and a uniform coarse all-seed theorem,
but the clone class is quantitatively excluded from near-minimality and
the coarse remainder is not subleading at the competitive scale.

The independent proof audit in
`twisted_chiral_uniform_final_audit_2026_09_19.md` also passes the director's
cycle-repair deletion and the adversarial track's same-child-orbit insertion,
including their two-sided near-order modulus for the optimized family.

The final bounded selector audit,
`twisted_chiral_stability_row_bound_audit_2026_09_19.md`, preserves an
actual full-sign high-energy Hall obstruction to row-only Brégman
relaxation, explicitly repairable by a support matching test. It also
passes the director's doubly-stochastic scaling/hypersimplex refinement.
An explicit Sylvester family of normalized cap1/2 has admissible
conditional quadruples with complete strong sector supports and exact
high-energy stability probability1. Thus a uniform per-quadruple
penalty cannot follow from low cap alone; the remaining child-pair
average is essential. None of these facts settles that average.

No uniform low-cap/minimizer selection theorem has been proved. In
particular a successful O(n)-defect twisted doubling estimate would still
need an order-coverage supplement such as (8) for a second multiplier or
the selected thinning statement (9).
