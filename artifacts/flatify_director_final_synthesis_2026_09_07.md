...
A final independently audited symmetric variant gives a global hollow
signing with |H_C(x)|<=sqrt(n)||Pperp x||^2/2 plus
O(n^(3/2)(r log n/n)^(1/4)+n^(101/80)) at every order.
[Proof](flatify_adversary_2026_09_07_symmetric_global_projector_localization.md),
[independent audit](flatify_independent_2026_09_07_symmetric_global_projector_audit.md).
One-gauge quadratic-chaos control replaces the rectangular two-gauge step;
an explicit squared-order symmetric Hadamard supplies all orders. This
really permits global edge changes, but its reference is a projected
Hadamard, not the weighted optimal-child target. It is not a sub-half
bound or favorable flatification.
...
[Exact recurrence accounting](flatify_director_quenched_recurrence_accounting_2026_09_07.md)
shows that a favorable Gaussian comparison with cap error N^(3/2-delta)
would give recurrence defect O(N^(1-delta)+N^(5/6)sqrt(log N)), uniformly
over comparable splits at fixed covariance gap. The Gaussian comparison
is a hypothesis, not a new theorem asserted about optimizing children.
