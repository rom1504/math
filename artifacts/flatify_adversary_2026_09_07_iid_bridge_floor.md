...
Date: 2026-09-07. Status: **derived proof; pending independent audit**.
...
beta(B)=max_(x,y Boolean)|x^TBy|. Then
    ...
    liminf_m E beta(B)/(2m)^(3/2) >= .71/sqrt(2) > .502.   (1)
...
For example beta=m^(1/12) makes this and the smoothing error tend to zero.
