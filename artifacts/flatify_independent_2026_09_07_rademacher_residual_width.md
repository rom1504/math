...
X_n be ANY prescribed real subspaces with rank r_n=o(sqrt(n)). There
...
||BB^T||F^2=||B||S4^4<=||A||S4^4=tr(A^4). By (3), the right side of
(8) is O(n^(3/2)).
...
For any orthogonal projection P_X of rank r,
    ...
    E||P_X x^+||^2 <=||P_X||F ||K||F <=C sqrt(r)n^(3/4),

and the same holds for x^-. Since r=o(sqrt(n)), both expectations are
...
For two low-cap actual children A,D and arbitrary rank-o(sqrt(n))
