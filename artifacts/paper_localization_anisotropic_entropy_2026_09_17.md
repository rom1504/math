...
**Later scope correction (18:32 UTC).** The finite entropy theorem above
remains valid. The whole-near-level profile (11) below is now known to be
impossible for every bounded positive-cap full-sign family: the fixed-radius
flip identity forces near-level entropy at least order eta log(1/eta),
whereas (11) would make it O(eta). This section is retained as a valid but
vacuous conditional implication, not a surviving optimizer construction.
The nonvacuous application uses low-response CENTER codes and pays their
Hamming deviations separately; see
[the anchored critical realization theorem](paper_localization_critical_realization_2026_09_17.md).
