...
The eventual problem is convergence of `M_n/n^(3/2)` for dense quadratic
signings.  This campaign does not attack it directly.  Reconnection requires
one of:

1. a dense-bridge response state strictly smaller than the full Boolean
   landscape;
2. a cross-scale theorem with `o(n^(3/2))` loss;
3. a strong incompressibility theorem excluding arbitrary dense bridge
   composition; or
...
- **Sharp projected-edge ceiling:** exact population isotropy and even
  `log K_2=o(r)` do not extend that closure by the same method.  A weak
  Walsh--Hadamard cluster mixture has
  `log K_2=Theta(r loglog(r)/log(r))`, yet multinomial occupancy creates any
  prescribed fixed super-Bai--Yin edge after every deterministic `o(r)`-rank
  peel.  This is a scalable falsifier of population-moment/projected-edge
  extensions, not a favorable-pressure law.
- **Exact compactness input:** tightness of the random latent component
  complexity `D_2(nu_(Z,r)||U_r)` is enough; bounded mean `D_2` is a simple
  sufficient condition.  Qualitative fixed-density convergence supplies a
  nonconstructive slowly growing window but no named rate.
- **Actual-child reset:** the exact negative escort of every contracted-
  temperature minimizing child pair has uniformly tight conditional row
  `D_2` after every prefix.  Its best independent-row shadow has the same
  component bound, and the omitted dependence is exactly
  `I^leftarrow=lambda(V^row-V)`.  Relative to a target `T`, a sublinear
  product excess `V^row-T` constructs a linearly rare near-target basin,
  whereas a fixed linear excess, if the full escort reaches `T`, certifies
  extensive directed dependence.
- **Actual-child falsifiers:** tight autoregressive components do not imply a
  latent-iid decomposition, and the conference no-gain observable does not
  transfer merely from a law decomposition.  The canonical reversed Gibbs
  latent is almost disjoint from an exact order-nine actual escort at low
  physical temperature.  Radial flip averages are only the absolute-energy
  histogram and collide on two actual order-eight optimizer classes with
  different overlaps and rank-one responses.
...
- **Live target:** control the scalar actual-child product target excess
  `Delta_N=V_lambda^row-T_N` from a nonradial statistic strictly smaller than
  the full bridge response table.  A power-saving upper bound activates the
  rare-basin recurrence; a fixed linear lower bound is the first exact
  irreducible-dependence theorem.
- **Promotion status:** upgraded to Level-5 structural contact, still not a
  convergence architecture.  Conditional complexity has been classified,
  so the branch earns one more actual-child phase.  Conference variants,
  radial averages, and generic mean-field decompositions are frozen.
...
Adversarial statistical mechanics has now made its first rigorous contact
with the exact optimized-child bridge law: conditional row complexity is
tight, and a target-relative theorem isolates the product target excess as
the only remaining directional scalar.  This is a real reset from the
conference surrogate, not yet a recurrence theorem.

The branch should continue only while it controls that scalar from genuine
nonradial child structure.  The exact row-product best-response oracle can
reconstruct the whole bridge table, so direct nonconvex evaluation is not a
low-information state.  No portfolio branch is yet primary for the original
problem, but adversarial statistical mechanics is the highest-value active
test for one further phase.
...
- **Strongest theorem:** response error is at most
  `||J||_1 ||A-B||_square`, even with a future that pins one rare labeling;
  weak regularity and rounding give finite block representatives at scale
  `n^2`.
- **Best benchmark:** generic random weighted Max-Cut has exponentially many
  exact responses but is uniformly replaced by density `1/2` up to
  `O(n^(3/2))=o(n^2)`.
- **Best falsifier:** an Alon--Naor rectangle witness becomes a pinned
  four-label optimized-response gap of the same order.
- **Scale ceiling:** at scale `n^(3/2)`, generic weak regularity needs
  `epsilon=o(n^(-1/2))` and may use exponentially many blocks.
...
|---|---|---|---|
| 1 | finite-width Ising | boundary table or strict anticipatory support | passed |
| 1 | separator Max-Cut/CSP | projective boundary response table | passed |
...
After every substantive result, ask whether the theory predicted a smaller
state or merely accepted one as a hypothesis.  Promote an orthogonal theory
only after a theorem, benchmark win, or imported mechanism survives an exact
falsifier.  Do not reconnect to the signing problem from finite numerics or a
fixed-rank result alone.
