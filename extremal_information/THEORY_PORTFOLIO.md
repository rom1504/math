...
- **Live target:** leave the conference projected-edge hierarchy.  Inspect
  the *actual* bridge Gibbs law generated by contracted-temperature child
  minimizers, and decide whether its conditional structure falls into a
  proved no-gain class or necessarily carries non-tight component `D_2` or
  irreducible row dependence.  Pressure direction, not another edge
  statistic, is the required observable.
- **Promotion status:** retain as a scoped diagnostic branch, not a
  convergence architecture.  Conference population-edge variants are
  frozen.  Full promotion requires a theorem about the child-induced law in
  the exact bridge identity.
...
Adversarial statistical mechanics has strictly reduced its structured
benchmark to a deeply irregular support edge, but the Hadamard-cluster
counterexample now shows that deterministic population peeling cannot close
the remaining singular window even under exact isotropy and sublinear row
`D_2`.  This is a real ceiling: the conference projected-edge hierarchy is
frozen rather than extended by more moment or growth-rate variants.

The branch remains useful only as a diagnostic of the exact child-induced
bridge law.  The next result must say whether that law has tight conditional
component complexity, irreducible extensive dependence, or a pressure-
specific cancellation invisible to the empirical edge.  Until then no new
speed theorem reaches actual contracted-temperature minimizers, and no
portfolio branch is primary for the original problem.
