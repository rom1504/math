...
## 7. A macroscopic response-packing corollary

The preceding exceptional word is not only a failure of a union bound.  It
gives an information lower bound for any summary required to answer a
declared family of two-state pressure continuations.

For two sign matrices `Y,Y'\in\{\pm1\}^{m\times n}`, use the future query
`L_Y` from (FC.21), with `C=Y`.  The same Jensen and variance estimates give

```math
E_{P_Y}L_Y-E_{P_{Y'}}L_Y
\ge ta\{mn-|\langle Y,Y'\rangle|\}
     -t\sqrt{mn}-\log2,                          \tag{FC.28}
```

where `P_Y=\bigotimes_i f_{y_i}`.  For every fixed `rho<1`, standard random
coding supplies:

- `exp(c_rho mn)` sign matrices with pairwise absolute inner product at
  most `rho mn`;
- `exp(c_rho(m+n))` **rank-one** sign matrices with the same property, by
  taking products of two constant-angle binary codes.

On balanced splits, (FC.28) separates every pair by `cN` in the response
pseudometric generated by the two-state pressures.  The equivariant
degree-`2d` approximants in (FC.24) retain this separation for one fixed
sufficiently large `d`.  Their entropy is direction-independent, so the
same packing holds for entropy-regularized responses.

### Corollary FC.2 (response information forced by future rank)

Any summary which answers all two-state pressure queries to error below a
fixed sufficiently small multiple of `N` must have at least

```math
\boxed{
\log|\operatorname {range}S|=\Omega(mn)}         \tag{FC.29}
```

on the unrestricted coefficient query class, and at least

```math
\boxed{
\log|\operatorname {range}S|=\Omega(m+n)}        \tag{FC.30}
```

even when every future coefficient matrix is rank one.  These are lower
bounds for the declared generic carrier model, not for the still smaller
response image generated by actual optimizing children.

This identifies the composition-created information precisely: a random
row observation transmits only `O(1)` Shannon information by (FC.20a), but
future rank-one alignment exposes `Omega(N)` bits of contextual identity.

## 8. Consequence for the smallest missing lemma
