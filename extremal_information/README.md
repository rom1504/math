...
The recognition/stability target is now partly resolved.  Approximate
idempotence has a sharp nearby-metric repair but can drift at long depth.
Depth-uniform control instead follows from one of two finite mechanisms:
compatible endpoint gauges telescope exactly, or a recurrent
small-projective-image word resets old error.  On the full projective domain,
an all-finite max-plus map has Hilbert coefficient zero or one, so no generic
intermediate contraction bridges those cases.  The remaining unifying target
is a canonical decomposition or obstruction theorem deciding whether an
approximate block lumping is generated by gauge-coboundary and reset pieces;
a static response cover alone cannot decide this.
