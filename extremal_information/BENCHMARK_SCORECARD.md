...
| Repeated Walsh-bridge graph composition | rooted relation form and rooted bilinear amalgamation for linear labels; Kronecker carrier for arbitrary truth tables | no classical extremal quotient supplied | yes; relative-involution, orbit, gluing, and Weyl-trace arguments independently audited | exact `O(k^2)` orbit quotient in every ambient dimension; associative cross state `(kappa,J,Z)`; `rs` cross-form and `r^2+O(1)` intersection bits can be necessary; pairwise data misses ternary relations; all unrooted spectra forget `Z` conditional on `(Gram,relations)` | strong pass: first strict Boolean-extremal quotient with an exact nonlocal composition law and a sharp spectral/rooted separation; the minimal unrooted scalar semantic quotient remains open |
