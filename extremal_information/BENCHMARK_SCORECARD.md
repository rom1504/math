...
| Repeated Walsh-bridge graph composition | rooted relation form for odd-dimensional linear labels; Kronecker carrier for arbitrary truth tables | no classical extremal quotient supplied | yes; relative-involution and orbit algebra independently audited | `(Gram,relations,omega-root)` uses `O(k^2)` bits and exactly classifies every `k`-label graph up to Walsh symmetry; Gram alone has a `((7-3sqrt3)/2)n^(3/2)` three-block gap; isolated rooted forms omit cross-data | first strict Boolean-extremal quotient for a nontrivial full-rank graph-composed family; dynamic gluing fibre remains active |
