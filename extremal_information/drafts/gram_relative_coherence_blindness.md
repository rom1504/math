...
original generator ports, individual product Rayleigh deficits, or an
average near-top condition.  The theorem below gives a scalable negative
answer at logarithmic port arity.  Its identical matrix block is the full
degree-one block; no broader fixed-degree claim is made here.
...
={a_1^2\over\rho_p}=O(p^{-1}).}                  \tag{CB.13}
```

The equality uses the degree-`p` coefficient, whose absolute value is
`a_1` by (CB.2).
...
At the motivating dense scale, take `H_*=sqrt(N)T_*` and `r=sqrt(N)`.
Then the prescribed majority selector witness has child deficits

```math
{\rho_p\over2}N^{3/2}
\quad\hbox{and at most}\quad
{a_1^2\over2}N^{3/2},                            \tag{CB.14a}
```

respectively.  These are witness losses in the robust certificate, not a
claim that the full Boolean trust optima are separated.
...
For the coherent construction of Section 2, this twirl is exactly the
diagonal construction:

```math
\overline A_{\rm coh}=A_{\rm diag}.              \tag{CB.17a}
```

Moreover

```math
\|A_{\rm coh}-\overline A_{\rm coh}\|_{op}
\ge1-\sum_Su_S^4
\ge1-{a_1^2\over\rho_p}.                         \tag{CB.17b}
```

Thus the symmetry diagnostic sees precisely the coherence hidden from all
individual deficits.
...
```math
\|A-\overline A\|_{op}
\le {1\over2}\sum_{i=1}^p\|[A,\rho(g_i)]\|_{op}
\le {p\gamma\over2}.                             \tag{CB.22}
```
